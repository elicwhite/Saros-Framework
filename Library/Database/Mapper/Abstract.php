...
class Library_Database_Mapper_Abstract
{
	...
	*/
	 ...
	 *	Constructor Method
	 */
	public final function __construct(Library_Database_Adapter_Interface $adapter, $adapterRead = null)
	{
		...
		if(null !== $adapterRead)
		{
			if($adapterRead instanceof Library_Database_Adapter_Interface)
			{
				$this->_adapterRead = $adapterRead;
			} else {
				throw new Library_Database_Exception("Secondary/Slave adapter must implement 'Library_Database_Adapter_Interface'");
			}
		}
		...
		if(!$this->_datasource)
		{
			throw new Library_Database_Exception('Datasource name must be defined - please define the $_datasource variable. This can be a database table name, collection or bucket name, a file name, or a URL, depending on your adapter.');
		}

		// Ensure fields have been defined for current table
		if(!$this->fields())
		{
			throw new Library_Database_Exception("Mapper fields must be defined");
		}
	...
	public function fields()
	{
		...
		else
		{
			...
			$gfields = $reflect->getProperties(ReflectionProperty::IS_PUBLIC)
			*/
	...
	public function get($primaryKeyValue = 0)
	{
		...
		if(!$primaryKeyValue) {
			$entity = new $this->_defaultEntity;
	...
	/**
	 * Get/Set the default entity class
	 *
	 * @param Library_Database_Entity|string $entity The default entity class to use
	 * @return string the name of the entity class to use if $entity was null
	 */
	public function entityClass($entity = null)
	{
		if (is_null($entity))
			return $this->_defaultEntity;

		$name = "";
		if ($entity instanceOf Library_Database_Entity)
			$name = get_class($entity);
		else
			$name = $entity;
			
		$this->_defaultEntity = get_class($entity);
	 ...
	 */
	public function getRelationsFor($entity)
	{
		$this->checkEntity($entity);
		...
		if(count($rels) > 0) {
			foreach($rels as $column => $relation) {
				...
				if(!$mapperName) {
					throw new Library_Database_Exception("Relationship mapper for '" . $column . "' has not been defined.");
				}
				...
				// Load relation class
				$relationClass = 'Library_Database_Relation_' . $relation['relation'];
	...
	public function getResultSet($stmt)
	{
		if($stmt instanceof PDOStatement) {
			...
			$stmt->setFetchMode(PDO::FETCH_CLASS, "Library_Database_Entity");
			...
			return new Library_Data_EntitySet($results, $resultsIdentities);

		} else {
			return array();
			//throw new Library_Database_Exception("Expected PDOStatement object");
		}
	}
	 ...
	 * @throws Library_Database_Exception
	 */
	public function query($sql, array $binds = array())
	{
		...
		if($stmt = $this->adapter()->prepare($sql)) {
			...
			throw new Library_Database_Exception("Unable to execute SQL query - failed to create prepared statement from given SQL");
		}
	...
	{
		$query = new Library_Database_Query($this);
		...
		foreach($entity->toArray() as $field => $value) {
			if($relationColumns && array_key_exists($field, $relationColumns) && (is_array($value) || is_object($value))) {
				foreach($value as $relatedRow) {
					...
					if($value instanceof Library_Database_Relation) {
						$relatedObj = $value;
					} else {
					...
					// Row object
					if($relatedRow instanceof Library_Database_Entity) {
						$relatedRowObj = $relatedRow;

						// Associative array
					} elseif(is_array($relatedRow)) {
						$relatedRowObj = new Library_Database_Entity($relatedRow);
					}
	...
	{
		if($conditions instanceof $this->_defaultEntity) {
			$conditions = array(
		...
		if(is_array($conditions)) {
			...
			throw new Library_Database_Exception("Conditions must be entity object or array, given '" . gettype($conditions) . "'");
		}
	}
